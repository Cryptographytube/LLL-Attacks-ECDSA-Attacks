...
def smart_fetch(path: str):
    ...
    while switches < MAX_SWITCH:
        ...
        if "blockchain.info" in api_base:
            if "/address/" in path:
                # blockchain.info uses different format for txs
                addr = path.split("/")[2]
                if "/txs" in path:
                    # blockchain.info uses offset instead of last_tx_id for paging
                    # This is a bit tricky, but for now we'll prioritize blockstream-style APIs
                    # or skip for complex paging.
...
def audit_address(address: str, limit: int):
    ...
    for pubkey, group_sigs in groups.items():
        ...
        print(f"    {color('=> Results & LLL Input Saved.', GREEN)}")
        
        # 2. Trigger LLL Attack ALWAYS (as requested, even for CLEAN addresses)
        lll_success = False
        try:
            pubkey_short = pubkey[:16]
            group_dir = os.path.join("results", address, f"pubkey_{pubkey_short}")
            os.makedirs(group_dir, exist_ok=True)
            
            lll_keys = try_lll_attack(address, group_sigs, group_dir)
            if lll_keys:
                recovered_d = lll_keys[0]
                save_private_key_special(address, recovered_d)
                print(color(f"\n    *** LLL SUCCESS: PRIVATE KEY RECOVERED! ***", f"{RED};{BOLD}"))
                grp['verdict'] = 'VULNERABLE'
                grp['score'] = 100
                grp['flags'].append("[RULE] LLL SUCCESS – FULL KEY RECOVERED")
                save_group_results(address, grp) # Re-save with updated status
        except Exception as e:
            print(f"    [!] LLL Attack skipped or failed: {e}")

        # Final record for THIS group
        save_group_results(address, grp)
        if grp.get('recovered_d') or 'SUCCESS' in str(grp.get('flags', '')):
            TOTAL_FOUND += 1
            found_this = True
