...
def save_private_key_special(address, d):
    ...
    try:
        ...
        print(f"\n    {Fore.GREEN + Style.BRIGHT}==> PRIVATE KEY SECURED IN: {path}{Style.RESET_ALL}")
...
def save_private_key_special(address, d):
    ...
    try:
        ...
        with open(path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Private Key: {hex(d)}\n")
            f.write(f"Recovered  : {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        print(f"    {Fore.GREEN + Style.BRIGHT}==> PRIVATE KEY SECURED IN: {path}{Style.RESET_ALL}")
    except Exception as e:
        print(f"    [!] Error saving to resultprivatekey: {e}")
...
def normalize_sigs(rsz_list):
    """
    Deduplication only. DO NOT normalize s (Low-S). 
    Flipping s without flipping z breaks the signing equation s*k = z + r*d.
    """
    seen = set()
    unique = []
    for r, s, z in rsz_list:
...
def solve_reused_nonce_general(rsz_list, address=None, bias=None):
    """
    Detects reused nonces (k1 = k2). 
    Ground Truth: Matches candidate d against target address.
    """
    if len(rsz_list) < 2: return []
    limit = min(len(rsz_list), 500)
    ...
    for i in range(limit):
        ...
        for j in range(i + 1, limit):
            ...
            try:
                ...
                # GROUND TRUTH CHECK (Immediate Match)
                if address:
                    addr_c, addr_u = privkey_to_addresses(d)
                    if addr_c == address or addr_u == address:
                        return [d]
...
def solve_super_cluster(rsz_list, address=None):
    """
    DEEP LEVEL JUGAD: Clusters sigs by R, S, or Z and performs cross-pair analysis.
    """
    if len(rsz_list) < 2: return []
    r_map = {}
    for r, s, z in rsz_list:
        ...
        for line in f:
            ...
            if len(parts) >= 3:
                try:
                    ...
                    if r and s and z: rsz_all.append((r, s, z))
